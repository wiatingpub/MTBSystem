...
func InsertFilm(film *entity.Film) error {
...
func UpdateFilm(film *entity.Film) error {
	_, err := db.Exec("UPDATE `film` SET `img` = ?,`length` = ?,`film_price`=?,`film_director`=?,`title_cn`=?,`title_en`=?,`type`=?,`film_drama`=?,`common_special`=?,`company_issued`=?,`country`=?,`r_year`=?,`r_month`=?,`r_day`=? where `movie_id`=?",
		film.Img, film.Length, film.FilmPrice, film.FilmDirector, film.TitleCn, film.TitleEn, film.Type, film.FilmDrama, film.CommonSpecial, film.CompanyIssued, film.Country, film.RYear, film.RMonth, film.RDay, film.MovieId)
	if err == sql.ErrNoRows {
		return nil
	}
	return err
